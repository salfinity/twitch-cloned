...
const SideMenu = () => {
  return (
    <div className='fixed w-14 sm:w-16 xl:w-[15rem] h-screen p-2 bg-neutral-100 overflow-x-scroll'>
      <div>
        <p className='hidden xl:flex uppercase font-bold text-sm pb-2'>
        ...
        <div key={index} className='inline-flex items-center w-full py-0'>
          ...
          <div className='hidden xl:flex justify-between w-full'>
            <div>
              ...
              <p className='text-gray-600'>{item.game_name}</p>
            </div>
      ...
      {top_channels.map((item, index) => (
        <div key={index} className='inline-flex items-center w-full py-[px]'>
          <div>
            <Image src={item.avatar} width='50' height='50' alt='/' />
          </div>
          <div className='hidden xl:flex justify-between w-full'>
